...
- **وضع كلود** — معالجة BiDi تلقائيّة لواجهة Claude Code ملء الشاشة. يُفعَّل تلقائيّاً
  **فقط حين يكون كلود نفسه قيد التشغيل**، فتبقى البرامج الأخرى التي تستخدم الشاشة البديلة
  ...
  العربيّة مسبقاً.
- **تسلسلات BiDi القياسيّة** ([توصية terminal-wg](https://terminal-wg.pages.freedesktop.org/bidi/)) —
  يمكن لأيّ برنامج أن يعلن عقده مع الطرفيّة بدل الاعتماد على كشف كلود بالاسم:
  `CSI 8 l` (وضع BDSM الصريح: «مخرجاتي بالترتيب البصريّ مسبقاً»، يفعّل محرّك
  التحويل البصريّ→المنطقيّ نفسه)، و`CSI 8 h` للعودة إلى الوضع الضمنيّ (الافتراضيّ:
  الطرفيّة تتولّى BiDi)، و`CSI 1 SP k` / `CSI 2 SP k` / `CSI 0 SP k`‏ (SCP) لفرض
  اتّجاه الفقرة: يسار→يمين / يمين→يسار / كشف تلقائيّ. تُعاد الأوضاع إلى الافتراضيّ
  عند مغادرة الشاشة البديلة أو عودة سطر الأوامر، فلا يترك برنامجٌ منهارٌ الطرفيّةَ
  في الوضع الخطأ.
...
كلّ ما يلي هذه النقطة خاصّ **بالتشغيل من المصدر** فقط (للمساهمين ومطوّري
الإضافات، أو من يفضّل `python EasyTer.py`).

## المتطلّبات (من المصدر)

كلّ ما تحتاجه قبل البدء، بالترتيب. جديد على أدوات ويندوز البرمجية؟ اقرأ
الملاحظات أسفل الجدول أيضاً — فهي تغطّي الأخطاء الشائعة. **لا حاجة لأيّ من
هذا إن استخدمت ملفّ exe أعلاه.**

| # | المتطلّب | رابط التحميل | لماذا | هل تثبّته أنت؟ |
|---|---|---|---|---|
| ١ | **ويندوز ١٠ (الإصدار ١٨٠٩ / البناء ١٧٧٦٣) أو أحدث، أو ويندوز ١١** | — | تعتمد عليه ConPTY التي بُنيت EasyTer فوقها؛ خاصّ بويندوز فقط | موجود على جهازك مسبقاً |
| ٢ | **اتّصال بالإنترنت** | — | لتنزيل الكود وحزم Python أدناه (مرّة واحدة فقط) | — |
| ٣ | **Python 3.10–3.14** مع pip (مُضمَّن في المثبّت) | [python.org/downloads](https://www.python.org/downloads/) | لتشغيل `install.bat` وEasyTer نفسه | **نعم — ابدأ بهذا** |
| ٤ | **مساحة تخزين حرّة ~٢٠٠ ميغابايت** | — | لـPython نفسه بالإضافة إلى PySide6/pywinpty/pyte/wcwidth | غالباً متوفّرة مسبقاً |
| ٥ | **Git** — اختياري | [git-scm.com/downloads](https://git-scm.com/downloads) | فقط إن استنسخت المستودع (`git clone`) بدل تنزيل ZIP | فقط إن اخترت طريقة Git |
| ٦ | **صلاحيات المسؤول (Administrator)** — اختياري | — | فقط إن نقلت EasyTer إلى `C:\EasyTer` وظهر خطأ صلاحيات | فقط إن طُلب منك |
| ٧ | **حزم Python:** `PySide6` (≥ 6.5)، و`pywinpty` (≥ 3.0.5)، و`pyte`، و`wcwidth` | — | لتشغيل EasyTer | **لا — `install.bat` يثبّتها لك** |
...
- `pywinpty` 3.0.5 يملك عجلات (wheels) جاهزة لـPython 3.10–3.14، فلا حاجة
  لمترجم (compiler) أو Visual Studio Build Tools.
- لست بحاجة لتثبيت حزم الصفّ رقم ٧ بنفسك — `install.bat` (أو
  `pip install -r requirements.txt`) يثبّتها لك تلقائياً.

تفحص EasyTer كلّ هذا عند الإقلاع أيضاً: إن كان Python قديمًا أو حزمةٌ ناقصة، تعرض
